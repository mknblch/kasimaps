...
data class ZoneEvent(val zone: String, val playerName: String, val server: String) : PlayerEvent(playerName)
    ...
    fun onEqEvent(event: EqEvent): PlayerEvent? {
        if (event.origin == Origin.EQLOG) {
            ...
            zoneRegex.matchEntire(event.text)?.run {
                return ZoneEvent(groupValues[1], event.character ?: "?", event.server ?: "?")
            }
